...
import { buildQualityAudit } from "@/lib/qa-audit";
...
export default function Home() {
  ...
  const [visualEvidence, setVisualEvidence] = useState<Array<{ name: string; source: string; type: "Imagem" }>>([]);
  ...
  const qualityAudit = useMemo(() => material ? buildQualityAudit(material) : null, [material]);
  ...
  return (
    <main className="qa-shell">
      ...
      {stage === "fonte" && <section className="workspace"><div className="section-heading"><div><p className="eyebrow">Etapa 01</p><h2>Texto bruto</h2></div><span className="status-badge">Leitura local</span></div><p className="section-lead">Cole o texto bruto do requisito. Nada é organizado antes da sua confirmação de leitura.</p><div className="import-row"><label className="button button-secondary">Ler XLSX<input type="file" accept=".xlsx,.xls,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" hidden disabled={busy} onChange={(event) => event.target.files?.[0] && readXlsx(event.target.files[0])} /></label><label className="button button-secondary">Adicionar imagem<input type="file" accept="image/*" hidden onChange={(event) => event.target.files?.[0] && addImage(event.target.files[0])} /></label><label className="button button-secondary">Ler log<input type="file" accept=".log,.txt,text/plain" hidden disabled={busy} onChange={(event) => event.target.files?.[0] && readLog(event.target.files[0])} /></label><button className="button button-quiet" onClick={clear}>Limpar</button></div><textarea aria-label="Texto de origem" className="source-editor" value={source} onChange={(event) => changeSource(event.target.value)} placeholder="Cole aqui o conteúdo original do requisito." />{visualEvidence.length > 0 && <div className="visual-evidence"><div><h3>Evidências visuais preservadas</h3><p>As imagens estão disponíveis para conferência. Nenhuma descrição foi criada automaticamente.</p></div><div className="visual-grid">{visualEvidence.map((item) => <figure key={`${item.name}-${item.source}`}><img src={item.source} alt={item.name} /><figcaption>{item.type}: {item.name}</figcaption></figure>)}</div></div>}<div className="source-meta"><span>{blocks} bloco(s) preservado(s)</span><span>{source.length.toLocaleString("pt-BR")} caracteres</span></div><div className="confirmation-box"><label><input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} /> Revisei a leitura integral do conteúdo apresentado.</label><button className="button button-primary" disabled={!source.trim() || !confirmed || busy} onClick={organize}>Organizar entregas</button></div></section>}

      {stage === "entregas" && material && <section className="workspace"><div className="section-heading"><div><p className="eyebrow">Etapa 02</p><h2>Organizar História</h2></div><span className="status-badge">{material.deliveries.length} entrega(s)</span></div><p className="section-lead">Cada bloco corresponde a uma parte reconhecida na fonte. Conteúdo não reconhecido permanece disponível para revisão.</p><div className="delivery-list">{material.deliveries.map((delivery, index) => <button key={delivery.id} className={`delivery-card ${selectedDelivery === index ? "is-active" : ""}`} onClick={() => setSelectedDelivery(index)}><span>Entrega {index + 1}</span><strong>{delivery.title}</strong><small>{delivery.scenarios.length} cenário(s)</small></button>)}</div><div className="source-preview"><h3>Material preservado da entrega</h3><pre>{currentDelivery?.sourceText}</pre></div>{currentDelivery?.stories && currentDelivery.stories.length > 0 && <div className="story-list"><h3>Histórias/requisitos identificados</h3>{currentDelivery.stories.map((story) => <article className="story-card" key={story.id}><strong>{story.title}</strong><span>{story.requirement.acceptanceCriteria.length} critério(s) de aceite · {story.scenarioIds.length} STEP(s)</span></article>)}</div>}{currentDelivery?.requirement && <div className="requirement-summary"><h3>Estrutura do requisito</h3><div className="requirement-grid"><div><strong>Como um</strong><p>{currentDelivery.requirement.userStory.asA.join("\n") || "Não informado no conteúdo de origem."}</p></div><div><strong>Eu quero</strong><p>{currentDelivery.requirement.userStory.iWant.join("\n") || "Não informado no conteúdo de origem."}</p></div><div><strong>Para que</strong><p>{currentDelivery.requirement.userStory.soThat.join("\n") || "Não informado no conteúdo de origem."}</p></div><div><strong>Critérios de aceitação</strong><p>{currentDelivery.requirement.acceptanceCriteria.join("\n") || "Não informado no conteúdo de origem."}</p></div><div><strong>Regras de negócio</strong><p>{currentDelivery.requirement.businessRules.join("\n") || "Não informado no conteúdo de origem."}</p></div><div><strong>Restrições técnicas</strong><p>{currentDelivery.requirement.technicalConstraints.join("\n") || "Não informado no conteúdo de origem."}</p></div><div><strong>Fluxos</strong><p>{currentDelivery.requirement.flows.join("\n") || "Não informado no conteúdo de origem."}</p></div><div><strong>Exceções</strong><p>{currentDelivery.requirement.exceptions.join("\n") || "Não informado no conteúdo de origem."}</p></div><div><strong>Elementos técnicos</strong><p>{currentDelivery.requirement.technicalElements.join("\n") || "Não informado no conteúdo de origem."}</p></div></div>{currentDelivery.requirement.gaps.length > 0 && <div className="requirement-gaps"><strong>GAPS E INDEFINIÇÕES</strong><ul>{currentDelivery.requirement.gaps.map((gap) => <li key={gap}>{gap}</li>)}</ul></div>}</div>}<div className="action-row"><button className="button button-secondary" onClick={() => setStage("fonte")}>Voltar à fonte</button><button className="button button-primary" onClick={() => setStage("steps")}>Gerar STEPs</button></div></section>}

      {stage === "steps" && material && <section className="workspace"><div className="section-heading"><div><p className="eyebrow">Etapa 03</p><h2>Gerar STEPs</h2></div><span className="status-badge">Todos os cenários da fonte</span></div><p className="section-lead">Os cenários exibem somente o que foi reconhecido na fonte. Ausências são registradas como gaps.</p><div className="action-row"><button className="button button-primary" disabled={!stepsText} onClick={() => copyText(stepsText, "Cenários STEP")}>Copiar todos os STEPs</button></div>{qualityAudit && <section className="quality-audit" aria-label="Auditoria da qualidade"><div className="section-heading"><div><p className="eyebrow">Auditoria da qualidade</p><h3>{qualityAudit.requirement.classification.toUpperCase()}</h3></div><span className="status-badge">{qualityAudit.traceability.percentage}% rastreabilidade</span></div><p>{qualityAudit.summary}</p><div className="audit-grid"><div><strong>STEPs</strong><span>Total: {qualityAudit.steps.total}</span><span>Completos: {qualityAudit.steps.complete}</span><span>Parciais: {qualityAudit.steps.partial}</span><span>Inconsistentes: {qualityAudit.steps.inconsistent}</span></div><div><strong>Cobertura documental</strong><span>Requisitos: {qualityAudit.coverage.requirementsIdentified}</span><span>Fluxos: {qualityAudit.coverage.flowsIdentified}</span><span>Exceções: {qualityAudit.coverage.exceptionsIdentified}</span><span>Regras: {qualityAudit.coverage.rulesIdentified}</span></div><div><strong>GAPS</strong><span>Total: {qualityAudit.coverage.gapsFound}</span><span>{Object.entries(qualityAudit.gaps.reduce<Record<string, number>>((counts, gap) => ({ ...counts, [gap.category]: (counts[gap.category] ?? 0) + 1 }), {})).map(([category, count]) => `${category}: ${count}`).join(" · ") || "Nenhum GAP consolidado."}</span></div><div><strong>Rastreabilidade</strong><span>{qualityAudit.traceability.fullyTraceable} cenário(s) integral(is)</span><span>{qualityAudit.traceability.partiallyTraceable} cenário(s) para conferência</span></div></div>{qualityAudit.attentionPoints.length > 0 && <div className="requirement-gaps"><strong>PONTOS DE ATENÇÃO</strong><ul>{qualityAudit.attentionPoints.map((point) => <li key={point}>{point}</li>)}</ul></div>}<p><strong>Resumo:</strong> {qualityAudit.summary}</p></section>}<div className="scenario-stack">{scenarios.length ? scenarios.map((scenario, index) => <article className="scenario-card" key={scenario.id}><div className="scenario-top"><span>STEP {scenarioNumber(scenario, index)}</span><span className={scenario.status === "pronto" ? "status-ok" : "status-gap"}>{scenario.status === "pronto" ? "Fonte completa" : "Há gaps"}</span></div><h3>{scenario.title}</h3><p className="reference">História/Requisito: {scenario.storyTitle ?? scenario.title}</p><p className="scenario-origin">Origem: Página {scenario.origin?.page ?? "não identificada"} — {scenario.origin?.excerpt || "Trecho não identificado."}</p><p className="reference">Referência: {scenario.reference}</p><div className="scenario-content"><h4>Pré-condições</h4><p>{renderList(scenario.preconditions)}</p><h4>Passos</h4><ol>{scenario.steps.length ? scenario.steps.map((item, itemIndex) => <li key={itemIndex}>{item}</li>) : <li>Não informado no conteúdo de origem.</li>}</ol><h4>Resultado esperado</h4><p>{renderList(scenario.expectedResult)}</p>{scenario.gaps.length > 0 && <><h4>Gaps e indefinições</h4><ul>{scenario.gaps.map((gap, gapIndex) => <li key={gapIndex}>{gap}</li>)}</ul></>}</div><button className="button button-secondary" onClick={() => copyText(formatScenario(scenario), "STEP")}>Copiar STEP</button></article>) : <div className="empty-state">Nenhum cenário estruturado foi reconhecido. A fonte original continua preservada na primeira etapa.</div>}</div><div className="action-row"><button className="button button-secondary" onClick={() => setStage("entregas")}>Voltar à história</button><button className="button button-primary" onClick={() => setStage("performance")}>Analisar Performance</button></div></section>}
      ...
      <details className="about-card"><summary>About</summary><p>Erika QA é uma ferramenta desenvolvida para apoiar Analistas de Qualidade na organização de histórias de usuário, geração estruturada de casos de teste e identificação preventiva de riscos.</p><p>Projetada para funcionar localmente, sem dependência obrigatória de serviços externos, priorizando produtividade, rastreabilidade, qualidade e manutenibilidade.</p></details><footer className="qa-footer">O que não está na fonte não vira requisito. O conteúdo permanece no dispositivo.</footer>
    </main>
  );
}
